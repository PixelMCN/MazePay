...
class WithdrawCommand extends Command {
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool {
        ...
        if (count($args) < 1) {
            $sender->sendMessage($this->plugin->getMessage("withdraw-usage"));
            return false;
        }
        ...
        // Withdraw money
        if (!$economyManager->withdraw($sender->getName(), $amount)) {
            $sender->sendMessage($this->plugin->getMessage("bank-insufficient"));
            return false;
        }
        
        $sender->sendMessage($this->plugin->getMessage("withdraw-success", [
